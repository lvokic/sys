...
RC ConjunctionExpr::get_value(const Tuple &tuple, Value &value) const
{
  ...
  for (const unique_ptr<Expression> &expr : children_) { // 多个条件以 and 连接
    rc = expr->get_value(tuple, tmp_value); // 这里会进行表达式的连接
    if (rc != RC::SUCCESS) {
    ...
    if ((conjunction_type_ == Type::AND && !bool_value) || (conjunction_type_ == Type::OR && bool_value)) {
      value.set_boolean(bool_value);      // and 遇到 false 则退出， or 遇到 true 则退出
      return rc;
    }
  }
  ...
  const std::unordered_map<std::string, std::string> & table_alias_map)
{
  ...
  if (is_single_table) {
    ...
    set_name(table_name_ + "." + field_name); 
  }
...
// Project 算子的 cell_at 会调用该函数取得聚集函数最后计算的结果,传入的 Tuple 就是 gropuby 中的 grouptuple
...
RC SysFuncExpr::get_func_date_format_value(const Tuple &tuple, Value &value) const
{
  ...
  for (size_t i = 0; i < strlen(cell_format_chars); i++) {
    ...
    if (65 <= cell_format_chars[i] && cell_format_chars[i] <= 122) {
      switch (cell_format_chars[i]) {
        case 'Y': {
          char tmp[5];
          snprintf(tmp, 5 * sizeof(char), "%d", year);
          result_date_str += tmp;
          break;
        }
        case 'y': {
          char tmp[5];
          snprintf(tmp, 5 * sizeof(char), "%d", year % 100);
        ...
        case 'm': {
          char tmp[3];
          snprintf(tmp, 3 * sizeof(char), "%d", month);
          if (0 <= month && month <= 9) {
            result_date_str += "0";
          }
          result_date_str += tmp;
        ...
        case 'D': {
          char tmp[3];
          snprintf(tmp, 3 * sizeof(char), "%d", day);
          result_date_str += tmp;
          if (11 <= day && day <= 13) {
            result_date_str += "th";
          } else {
