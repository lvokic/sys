...
static void wildcard_fields(const BaseTable *table, const std::string &alias,
...
RC SelectStmt::process_from_clause(Db *db, std::vector<BaseTable *> &tables,
    std::unordered_map<std::string, std::string> &table_alias_map, std::unordered_map<std::string, BaseTable *> &table_map,
    std::vector<InnerJoinSqlNode> &from_relations, std::vector<JoinTables> &join_tables)
{
  std::unordered_set<std::string> table_alias_set;  // 检测别名是否重复
...
RC SelectStmt::create(
    Db *db, SelectSqlNode &select_sql, Stmt *&stmt, const std::unordered_map<std::string, BaseTable *> &parent_table_map)
{
  if (nullptr == db) {
  ...
  if (has_aggr_func_expr || select_sql.groupby_exprs.size() > 0) {
    ...
    std::vector<std::unique_ptr<Expression>> field_exprs_not_aggr;  // select 后的所有非 aggrexpr 的
    ...
    // 用于从 project exprs 中提取所有 field expr,
